(function(){
  ...
  GameView.prototype.startGame = function(){
    ...
    var game = this.game;
    this.stage.handleTick = function(){
      game.handleTick();
    };
    this.game.play();
  };
  ...
  GameView.prototype.endGame = function(){
    this.game = undefined;
    this.$viewPort.find("overlay").css({"visibility": "visible", "opacity": "1"});
  };
  ...
  GameView.prototype.resetWave = function(){
    this.$viewPort.find("#wave").html(1);
  };

  GameView.prototype.updateHighScores = function(){
    var gameView = this;

    this.getHighScores(function(scores){
      gameView.setHighScores(scores)
    });
  };

  GameView.prototype.setHighScores = function(scores){
    scores.sort().reverse();

    var highScores = this.$viewPort.find("#high-scores");
    this.$viewPort.find("#high-scores").find("li").each(function(idx, el){
      $(el).find("name").html(scores[idx][1]);
      $(el).find("score").html(scores[idx][0]);
    });
  };

  GameView.prototype.getHighScores = function(successCallback){
    var gameView = this;

    $.getJSON("assets/highscores.json", function(data){
